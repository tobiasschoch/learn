...
IMPORTANT: R follows the mathematical convention that multiplication and division precede addition and subtraction. For example, the following code first multiplies $2$ by $3$ and only after adds $4$.
...
The next code example shows how to compute the power of a number. For example, we want to compute $3^9$, which can be easily done in R with the following code:
...
The `print()` function prints the output of the variable that is provided as a function argument. So, let's see what the current value of the variable `a` is:
